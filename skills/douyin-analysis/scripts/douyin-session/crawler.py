...
async def _safe_screenshot(page: Page, path: Path) -> None:
    try:
        ...
        print(f"[诊断] 截图失败，继续执行：{exc}")


def _should_capture_detail_response(resp: Response) -> bool:
    url = resp.url
    if not any(host in url for host in ("creator.douyin.com", "creator.amemv.com")):
        return False
    blocked = (
        "monitor", "mcs.", "slardar", "passport", "msg/top", "user_message",
        "anchor/search", "xigua/user", "resource/upload", "workspace/feature",
        "abtest", "charge/", "banner/info", "im/token", "sts2",
    )
    if any(token in url for token in blocked):
        return False
    wanted = (
        "data", "media", "item", "work", "aweme", "stat", "analysis",
        "following", "creator", "comment",
    )
    return any(token in url for token in wanted)
...
async def _page_text(page: Page) -> str:
    ...
    for frame in page.frames:
        try:
            ...
            if text.strip() and text not in chunks:
                chunks.append(text)
        except Exception:
            pass
    return "\n\n".join(chunks)
...
class Session:
    ...
    async def open(cls, headless: bool = False) -> Session:
        ...
        chrome_executable = os.environ.get("DOUYIN_CHROME_EXECUTABLE", "").strip()
        if not chrome_executable:
            candidate = Path("/Applications/Google Chrome.app/Contents/MacOS/Google Chrome")
            if candidate.exists():
                chrome_executable = str(candidate)
        if chrome_executable:
            launch_options["executable_path"] = chrome_executable
        ctx = await pw.chromium.launch_persistent_context(**launch_options)
        return cls(ctx, pw)
...
async def fetch_recent_videos(sess: Session, limit: int = 50) -> list[dict]:
    ...
    try:
        ...
        detail_wait_s = int(os.environ.get("DOUYIN_DETAIL_WAIT_S", "8"))
        await asyncio.sleep(detail_wait_s)
        # 翻页加载更多
        for _ in range(3):
...
async def fetch_video_detail(sess: Session, aweme_id: str) -> dict:
    ...
    async def on_response(resp: Response) -> None:
        all_urls.append(resp.url)
        if _should_capture_detail_response(resp):
            try:
                data = await resp.json()
                captured.append({"url": resp.url, "data": data})
            except Exception:
    ...
    page.on("response", on_response)
    try:
        url = f"https://creator.douyin.com/creator-micro/data/stats/video/{aweme_id}"
        ...
        debug_path = debug_dir()
        debug_path.mkdir(parents=True, exist_ok=True)
        (debug_path / f"detail_text_{aweme_id}.txt").write_text(text, encoding="utf-8")
        if os.environ.get("DOUYIN_DEBUG_SCREENSHOTS"):
            await _safe_screenshot(page, debug_path / f"detail_page_{aweme_id}.png")
        if not captured:
            (debug_path / "detail_urls.txt").write_text("\n".join(all_urls), encoding="utf-8")
            print("[诊断] 详细数据接口没拦到，已 dump URL 列表。")
        return {"captured": captured, "text": text}
    finally:
        await page.close()
...
async def fetch_comments_creator(sess: Session, aweme_id: str, max_pages: int = 60) -> list[dict]:
    ...
    try:
        ...
        debug_path.mkdir(parents=True, exist_ok=True)
        if os.environ.get("DOUYIN_DEBUG_SCREENSHOTS"):
            await _safe_screenshot(page, debug_path / "creator_comment_page.png")
...
async def fetch_comments(sess: Session, aweme_id: str, max_pages: int = 60) -> list[dict]:
    ...
    try:
        ...
        debug_path.mkdir(parents=True, exist_ok=True)
        if os.environ.get("DOUYIN_DEBUG_SCREENSHOTS"):
            await _safe_screenshot(page, debug_path / "comment_page.png")
